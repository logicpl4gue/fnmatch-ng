/* compat/musl_fnmatch.c - vendored musl reference implementation.
 *
 * Verbatim copy of musl 1.2.5 src/regex/fnmatch.c ("Sea of Stars"
 * algorithm, Rich Felker). MIT license; see compat/README.md.
 *
 * Local modifications only (all documented):
 *   1. <fnmatch.h>        -> "musl_fnmatch.h"  (self-contained include)
 *   2. "locale_impl.h"    -> MB_CUR_MAX shim below. locale_impl.h is musl
 *      internal; this file uses nothing from it except MB_CUR_MAX. Outside
 *      musl we cannot evaluate the current locale, and the documented M0
 *      scope is the C/POSIX locale, where MB_CUR_MAX is 1. Forcing 1 keeps
 *      the tail-scan behavior identical to musl running in the C locale.
 *   3. Provenance header added; no algorithm or token changed.
 *   4. GCC diagnostic pragma to silence upstream -Wsign-compare noise.
 */
...
/* shim for locale_impl.h, see provenance header above */
#ifdef MB_CUR_MAX
#undef MB_CUR_MAX
#endif
#define MB_CUR_MAX 1

/* 4. diagnostic noise from upstream code when built with -Wall -Wextra
 *    outside musl's own build system */
#if defined(__GNUC__)
